...
#Forecasting with two-step DNS, VAR(1).
# the fitting is done by the method of moments method assuming stationarity
#INPUT : ts = table of OLS fitted factors (l,s,c) per cross-section (they are generated by ts_DNS_creator_daily.py). pred =  number of forecasts ahead (in days)
#OUTPUT : table with pred rows. Row i is the factor forecast i days ahead of the last date in ts.
def forecast_DNS_VAR_yw(ts, pred):
    
    T, n_ts = ts.shape
    mu = np.mean(ts, axis=0)
    
    cov0 = 1/T *  (ts - mu).transpose() @ (ts - mu)
    cov1 = 1/T *  (ts[1:,:] - mu).transpose() @ (ts[0:-1,:] - mu)
    A = cov1 @ np.linalg.inv(cov0)
    
    # forecasts
    step_two_VAR_new = np.zeros([pred, n_ts])
    step_two_VAR_new[0,:] = mu + A@(ts[-1,:]-mu)
    for t in range(1,pred):
        step_two_VAR_new[t,:] = mu + A @ (step_two_VAR_new[t-1,:]-mu)
        
    return(step_two_VAR_new)
...
#Forecasting with VAR(1) using the OLS fit.
#INPUT : da = sample of data used for the estimation. pred =  number of forecasts ahead (in days)
#OUTPUT : table with pred rows. Row i is the yield curve forecast i days ahead of the last date in da.
...
#Forecasting with VAR(1) using the Yule Walker fitting method.
#INPUT : da = sample of data used for the estimation. pred =  number of forecasts ahead (in days)
#OUTPUT : table with pred rows. Row i is the yield curve forecast i days ahead of the last date in da.
def forecast_VAR_yw(da, pred):
    
    # extract size parameters
    T, n_matu = da.shape
    mu = np.mean(da, axis = 0)
    
    # yule-walker estimation of the transition matrix
    cov0 = 1/T *  (da - mu).transpose() @ (da - mu)
    cov1 = 1/T *  (da[1:,:] - mu).transpose() @ (da[0:-1,:] - mu)
    A = cov1 @ np.linalg.inv(cov0)
    
    # forecasting
    forecast = np.zeros([pred, n_matu])
    forecast[0,:] = mu + A@(da[-1,:]-mu)
    for t in range(1,pred):
        forecast[t,:] = mu + A @ (forecast[t-1,:]-mu)
        
    return(forecast)

#Forecasting with AR(1) using the OLS fit.
#INPUT : da = sample of data used for the estimation. pred =  number of forecasts ahead (in days)
#OUTPUT : table with pred rows. Row i is the yield curve forecast i days ahead of the last date in da.
...
#%% Forecasting with AR(1) with Yule Walker equations.
#INPUT : da = sample of data used for the estimation. pred =  number of forecasts ahead (in days)
#OUTPUT : table with pred rows. Row i is the yield curve forecast i days ahead of the last date in da.
def forecast_AR_yw(da, pred):
    ...
    # force the matrices to be diagonal, therefore it is going to be AR at each maturity
...
# Forecasting with one-step DNS with explosivity correction
# if the estimated transition matrix corresponds to an explosive VAR process, return the RW forecast instead
#INPUT : da = sample of data used for the estimation. tau = vector of maturities. lamb = lambda factor estimated. state_init = initialisation of the states means for KF .pred =  number of forecasts ahead (in days)
#OUTPUT : table with pred rows. Row i is the yield curve forecast i days ahead of the last date in ts.
def forecast_DNS_KF_explosivcor(da,tau,lamb,state_init,offset_init,trans_init,pred):
    ...
    offs = kf.transition_offsets
    if (np.linalg.norm( np.linalg.matrix_power(A,100000), ord=2)) > 1:
    ...
    last_value = np.array([smoother_x[-1]])    
    for i in range(pred):
        forecast[i,:] = np.array(last_value @ A.transpose() + offs)
        last_value = forecast[i,:]
    return forecast
